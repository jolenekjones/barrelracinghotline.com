             ...
             <li>
                 @if (
                     (auth()->check() &&
                         ...
                         (auth()->check() && auth()->user()->id == $horse->customerid))
                     <a class="dropdown-item" href="{{ env('HOTLINEWEBSITE_LINK') }}/horsehotline/manage/horses/{{ $horse->id }}/edit">
                         <span data-lucide="pencil"></span>&nbsp;Edit Horse
                     </a>
                 @elseif(auth()->check())
                 ...
                 @else
                     <a class="dropdown-item login"
                         href="{{ env('HOTLINEWEBSITE_LINK') }}?site-url={{ url()->current() }}">
                         <span data-lucide="pencil"></span>&nbsp;Edit Horse
                     </a>
                 @endif
             </li>
             <li>
                 <div class="dropdown-item" onclick="makeItFeature({{ $horse->id }})">
                     <span data-lucide="star"></span>&nbsp;Feature It
                 </div>
             </li>
             <li>
                 <div class="dropdown-item text-danger cursor-pointer"
                     onclick="confirmHorseAction({{ $horse->id }})">
                     <span data-lucide="trash"></span>&nbsp; Delete / Draft
                 </div>
             </li>
             <li>
                 <a class="dropdown-item" href="#">
                     <span data-lucide="share-2"></span>&nbsp;Share Horse
                 </a>
             </li>
         </ul>
     </div>
 @push('scripts')
     ...
     <script>
         ...
         /** Delete/Draft Action **/
         function confirmHorseAction(horseId) {
             if (!window.horsePermissions.canManage) {
                 Swal.fire('Not Allowed!', 'You are not allowed to perform this action.', 'error');
             } else {
                 Swal.fire({
                     title: 'Are you sure?',
                     text: "You want to delete this horse.",
                     icon: 'warning',
                     showCancelButton: true,
                     showDenyButton: true,
                     ...
                     if (result.isConfirmed) {
                         // Call delete route
                         $.ajax({
                             url: `/horses/${horseId}/delete`, // your delete route
                             type: 'POST',
                             data: {
                                 _token: $('meta[name="csrf-token"]').attr('content')
                             },
                             ...
                             success: function(response) {
                                 Swal.fire('Deleted!', 'The horse has been deleted.', 'success')
                                     .then(() => {
                                         location.reload(); // refresh page after alert is closed
                                     });
                                 // reload page or remove row
                         ...
                         // Call draft route
                         $.ajax({
                             url: `/horses/${horseId}/draft`, // your draft route
                             type: 'POST',
                             data: {
                                 _token: $('meta[name="csrf-token"]').attr('content')
                             },
                             ...
                             success: function(response) {
                                 Swal.fire('Saved!', 'The horse has been moved to draft.', 'success')
                                     .then(() => {
                                         location.reload(); // refresh page after alert is closed
                                     });
                             },
